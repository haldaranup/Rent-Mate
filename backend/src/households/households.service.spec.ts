...
describe('HouseholdsService', () => {
  ...
  beforeEach(async () => {
    ...
    householdRepository = module.get<MockRepository<Household>>(getRepositoryToken(Household));
  ...
  describe('create', () => {
    ...
    it('should successfully create a household and assign user as owner', async () => {
      const userNotInHousehold = { ...mockCreatingUser, householdId: null, household: null };
      const newHouseholdEntity = { id: 'new-household-id', ...createHouseholdDto, members: [], chores: [] };
      const populatedHousehold = { ...newHouseholdEntity, members: [{...userNotInHousehold, role: UserRole.OWNER, householdId: newHouseholdEntity.id}] };
      ...
      const result = await service.create(createHouseholdDto, userNotInHousehold);

      expect(householdRepository.create).toHaveBeenCalledWith(expect.objectContaining(createHouseholdDto));
    ...
    it('should throw ConflictException if user is already in a household', async () => {
      const userInHousehold = { ...mockCreatingUser, householdId: mockExistingHousehold.id, household: mockExistingHousehold };
      householdRepository.findOne!.mockResolvedValue(mockExistingHousehold); // For the check if user is in household

      await expect(service.create(createHouseholdDto, userInHousehold))
        .rejects.toThrow(ConflictException);
      expect(householdRepository.findOne).toHaveBeenCalledWith({ where: { id: userInHousehold.householdId } });
    });
    ...
    it('should allow creating a household if user.householdId is set but household findOne returns null (data inconsistency)', async () => {
      const userWithOrphanedHouseholdId = { ...mockCreatingUser, householdId: 'orphan-id', household: null };
      const newHouseholdEntity = { id: 'new-household-id-2', ...createHouseholdDto, members: [], chores: [] };
      const populatedHousehold = { ...newHouseholdEntity, members: [{...userWithOrphanedHouseholdId, role: UserRole.OWNER, householdId: newHouseholdEntity.id}] };
      ...
      householdRepository.findOne!.mockResolvedValueOnce(populatedHousehold as any); // Second findOne (for refetch)

      const result = await service.create(createHouseholdDto, userWithOrphanedHouseholdId);
      expect(result).toEqual(populatedHousehold);
    ...
    it('should throw InternalServerErrorException if household fails to be retrieved after creation', async () => {
      const userNotInHousehold = { ...mockCreatingUser, householdId: null, household: null };
      const newHouseholdEntity = { id: 'new-household-id-fail', ...createHouseholdDto, members: [], chores: [] };
      ...
      await expect(service.create(createHouseholdDto, userNotInHousehold))
  ...
  describe('findOneByIdWithMembers', () => {
    it('should return a household with members if found', async () => {
      const householdWithMembers = { ...mockExistingHousehold, members: [mockCreatingUser] };
      householdRepository.findOne!.mockResolvedValue(householdWithMembers);

      const result = await service.findOneByIdWithMembers(mockExistingHousehold.id);
      expect(householdRepository.findOne).toHaveBeenCalledWith({
