...
export class HouseholdsService {
  ...
  async create(
    ...
    if (creatingUser.householdId) {
      const existingHousehold = await this.householdsRepository.findOne({
        where: { id: creatingUser.householdId },
      });
      if (existingHousehold) {
        throw new ConflictException(
          `User is already in household: ${existingHousehold.name}`,
        );
      }
    ...
    const savedHousehold =
      await this.householdsRepository.save(newHouseholdEntity);
    ...
    if (!populatedHousehold) {
      ...
      throw new InternalServerErrorException(
        'Failed to retrieve household after creation.',
      );
    }
    ...
    await this.activityLogService.createLogEntry({
      ...
      details: {
        householdName: populatedHousehold.name,
        ownerUserId: creatingUser.id,
      },
    });
  ...
  async findOneByIdWithMembersAndValidateMembership(
    householdId: string,
    userId: User['id'],
  ): Promise<Household> {
    const household = await this.householdsRepository.findOne({
  ...
  async removeMemberFromHousehold(
    ...
    if (requestingUser.id === memberUserIdToRemove) {
      throw new ConflictException(
        'Household owner cannot remove themselves. Use delete household or transfer ownership (not implemented).',
      );
    }

    // 3. Find the member to be removed
    const memberToRemove =
      await this.usersService.findOneById(memberUserIdToRemove);
    if (!memberToRemove) {
    ...
    if (memberToRemove.householdId !== householdId) {
      throw new ConflictException(
        'This user is not a member of your household.',
      );
    }
