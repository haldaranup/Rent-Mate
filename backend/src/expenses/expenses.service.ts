...
export class ExpensesService {
  ...
  async create(createExpenseDto: CreateExpenseDto, creatingUser: User): Promise<Expense> {
    const { paidById, shares, amount, description, date } = createExpenseDto; // Removed householdId from here

    if (!creatingUser.householdId) {
      throw new ForbiddenException('You must belong to a household to create an expense.');
    }
    ...
    try {
      ...
      const household = await this.householdsService.findOneByIdWithMembers(householdId);
      if (!household) {
      ...
      if (paidById) {
        ...
        if (!paidByUser) {
          throw new BadRequestException(`User specified as payer with ID ${paidById} is not a member of your household or does not exist.`);
        }
      } else {
      ...
      if (!shares || shares.length === 0) {
        throw new BadRequestException('Expense shares must be provided and cannot be empty.');
      }

      for (const shareDto of shares) {
        // Ensure owedById from shareDto is a member of the user's household
        const shareUser = household.members.find(member => member.id === shareDto.owedById);
        if (!shareUser) {
          throw new BadRequestException(`User with ID ${shareDto.owedById} in shares is not a member of your household or does not exist.`);
        }
      ...
      if (Math.abs(totalSharesAmount - Number(amount)) > tolerance) {
        throw new BadRequestException(`Sum of share amounts (${totalSharesAmount.toFixed(2)}) does not match total expense amount (${Number(amount).toFixed(2)}).`);
      }
  ...
  async findAllForUserHousehold(requestingUser: User): Promise<Expense[]> {
    ...
    const household = await this.householdsService.findOneByIdWithMembers(householdId);
    if (!household) {
      throw new NotFoundException(`Household with ID ${householdId} not found. This may indicate data inconsistency.`);
    }
    // Double check user membership for sanity, though householdId comes from user token
    const isUserMember = household.members.some(member => member.id === requestingUser.id);
    if (!isUserMember) {
      // This should ideally never happen if user.householdId is correctly set from a valid JWT
      throw new ForbiddenException('You are not a member of the household associated with your profile.');
    }
  ...
  async findOne(expenseId: string, requestingUser: User): Promise<Expense> {
    ...
    if (!expense.household) {
      ...
      throw new InternalServerErrorException('Expense is not associated with a household.');
    }

    const isUserMember = expense.household.members.some(member => member.id === requestingUser.id);
    if (!isUserMember) {
  ...
  async update(expenseId: string, updateExpenseDto: UpdateExpenseDto, user: User): Promise<Expense> {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    ...
    try {
      ...
      if (expense.householdId !== user.householdId) { // Check against user's household
        throw new ForbiddenException('You are not authorized to update this expense for this household.');
      }
      // Already checked that user is member of *their* household implicitly by user object having householdId

      const { shares: newSharesDto, paidById: newPaidById, amount: newAmount, ...otherUpdates } = updateExpenseDto;

      // Update basic fields like description, date
      if (otherUpdates.description !== undefined) expense.description = otherUpdates.description;
      ...
      if (newPaidById !== undefined) {
        if (newPaidById === null) { 
          ...
          throw new BadRequestException('paidById cannot be null. An expense must have a payer.');
          // expense.paidBy = null;
          // expense.paidById = null;
        } else {
          const paidByUser = expense.household.members.find(member => member.id === newPaidById);
          if (!paidByUser) {
            throw new BadRequestException(`New payer with ID ${newPaidById} is not a member of this household or does not exist.`);
          }
      ...
      const currentExpenseAmount = newAmount !== undefined ? Number(newAmount) : expense.amount;
      if (newAmount !== undefined) {
         expense.amount = Number(newAmount);
      }

      // If shares are provided, replace existing ones
      if (newSharesDto) { // Check if shares array is part of the DTO (even if empty)
        if (newSharesDto.length === 0) {
            throw new BadRequestException('Expense shares cannot be empty if provided for update.');
        }
        let totalNewSharesAmount = 0;
        for (const shareDto of newSharesDto) {
          const shareUser = expense.household.members.find(m => m.id === shareDto.owedById);
          if (!shareUser) throw new BadRequestException(`User with ID ${shareDto.owedById} in shares is not a member of this household or does not exist.`);
          totalNewSharesAmount += Number(shareDto.amountOwed);
        }

        const tolerance = 0.001;
        if (Math.abs(totalNewSharesAmount - currentExpenseAmount) > tolerance) {
          throw new BadRequestException(`Sum of new share amounts (${totalNewSharesAmount.toFixed(2)}) does not match total expense amount (${currentExpenseAmount.toFixed(2)}).`);
        }
        ...
        const effectivePaidById = newPaidById !== undefined ? newPaidById : expense.paidById;
        if (!effectivePaidById) {
            // This should not happen if paidById is always required.
            throw new InternalServerErrorException('Cannot determine payer for settling shares.');
        }
      ...
      } else if (newAmount !== undefined && expense.shares && expense.shares.length > 0) {
        ...
        throw new BadRequestException('If expense amount is updated, new shares must also be provided.');
      }
  ...
  async toggleExpenseShareSettlement(
    ...
    try {
      ...
      if (!expenseShare) {
        throw new NotFoundException(`Expense share with ID ${shareId} not found.`);
      }
      ...
      await this.activityLogService.createLogEntry({
        ...
        activityType: settle ? ActivityType.EXPENSE_SHARE_SETTLED : ActivityType.EXPENSE_SHARE_UNSETTLED,
        details: {
          expenseDescription: expenseShare.expense.description,
          expenseId: expenseShare.expenseId,
          owedByUserId: expenseShare.owedById,
          owedByUserName: expenseShare.owedBy?.name || expenseShare.owedBy?.email,
  ...
  async getHouseholdBalances(householdId: string, requestingUser: User): Promise<UserBalance[]> {
    const household = await this.householdsService.findOneByIdWithMembersAndValidateMembership(householdId, requestingUser.id);
    ...
    for (const member of household.members) {
      ...
      const totalPaid = paidExpenses.reduce((sum, expense) => sum + Number(expense.amount), 0);
      ...
      const owedShares = await this.expenseSharesRepository.find({
        where: { owedById: member.id, isSettled: false, expense: { householdId: household.id } },
        relations: ['expense'], // ensure expense relation is loaded for householdId check
      });
      const totalOwed = owedShares.reduce((sum, share) => sum + Number(share.amountOwed), 0);
  ...
  async getSettleUpSuggestions(householdId: string, requestingUser: User): Promise<SettleUpSuggestion[]> {
    const balances = await this.getHouseholdBalances(householdId, requestingUser);
    ...
    while (debtorIndex < debtors.length && creditorIndex < creditors.length) {
      ...
      if (amountToSettle > 0.001) { // Only add suggestion if amount is significant
        suggestions.push({
      ...
      if (debtor.netBalance < 0.001) { // Debtor is settled
        debtorIndex++;
      }
      if (creditor.netBalance < 0.001) { // Creditor is settled
        creditorIndex++;
      }
    }
    return suggestions;
