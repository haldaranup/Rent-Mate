...
export class InvitationsService {
  ...
  private async generateShortCode(): Promise<string> {
    for (let i = 0; i < SHORT_CODE_MAX_RETRIES; i++) {
      ...
      for (let j = 0; j < SHORT_CODE_LENGTH; j++) {
        code += SHORT_CODE_CHARSET.charAt(
          Math.floor(Math.random() * SHORT_CODE_CHARSET.length),
        );
      }
    ...
    this.logger.error(
      `Failed to generate a globally unique short code after ${SHORT_CODE_MAX_RETRIES} retries.`,
    );
  ...
  async createEmailInvitation(
    ...
    if (
      invitingUser.householdId !==
      householdId /* && invitingUser.role !== UserRole.OWNER */
    ) {
    ...
    const existingUserInHousehold =
      await this.usersService.findOneByEmail(inviteeEmail);
    if (
      existingUserInHousehold &&
      existingUserInHousehold.householdId === householdId
    ) {
      this.logger.warn(
  ...
  async createHouseholdShortCodeInvitation(
    ...
    // Return the full invitation object including the shortCode
    this.logger.log(
    ...
    if (
      invitingUser.householdId !== householdId &&
      invitingUser.role !== UserRole.OWNER
    ) {
      this.logger.warn(
    ...
    await this.invitationsRepository.save(newInvitation);
    this.logger.log(
      `Short code invitation created with ID: ${newInvitation.id}, Code: ${shortCode}`,
    );
  ...
  private async processAcceptance(
    ...
    if (
      invitation.email &&
      invitation.email.toLowerCase() !== acceptingUserEmail.toLowerCase()
    ) {
      this.logger.warn(
    ...
    const userToUpdate =
      await this.usersService.findOneByEmail(acceptingUserEmail);
    if (!userToUpdate) {
      this.logger.warn(
        `User with email ${acceptingUserEmail} not found during invitation acceptance.`,
      );
      ...
      throw new HttpException(
        'User to accept invitation not found',
    ...
    if (userToUpdate.householdId) {
      if (userToUpdate.householdId === invitation.householdId) {
        this.logger.warn(
          `User ${acceptingUserEmail} is already a member of household ${invitation.householdId}.`,
        );
        ...
        this.logger.warn(
          `User ${acceptingUserEmail} is already a member of another household (${userToUpdate.householdId}).`,
        );
        throw new ConflictException(
    ...
    this.logger.log(
      `User ${acceptingUserEmail} added to household ${invitation.householdId}`,
    );
  ...
  async acceptInvitationByToken(
    ...
    this.logger.log(
      `User ${acceptingUser.email} attempting to accept invitation by TOKEN ${token}`,
    );
  ...
  async acceptInvitationByShortCode(
    shortCode: string,
    acceptingUser: User,
  ): Promise<Household> {
    this.logger.log(
      `User ${acceptingUser.email} attempting to accept invitation by SHORT CODE ${shortCode}`,
    );
    ...
    if (!invitation) {
      ...
      throw new HttpException(
        'Invitation code not found, invalid, or already used',
        HttpStatus.NOT_FOUND,
      );
    }
  ...
  async declineInvitationByToken(
    ...
    this.logger.log(
      `User ${decliningUser.email} attempting to DECLINE invitation by TOKEN ${token}`,
    );
    const invitation = await this.findByToken(token);
    ...
    if (invitation.status !== InvitationStatus.PENDING) {
      this.logger.warn(
        `Invitation ${invitation.id} is not pending (status: ${invitation.status}). Cannot decline.`,
      );
      ...
      throw new HttpException(
        `Invitation is no longer pending and cannot be declined (current status: ${invitation.status})`,
    ...
    if (new Date() > invitation.expiresAt) {
      this.logger.warn(
        `Invitation ${invitation.id} has expired. Marking as EXPIRED instead of DECLINED.`,
      );
      invitation.status = InvitationStatus.EXPIRED;
      await this.invitationsRepository.save(invitation);
    ...
    await this.invitationsRepository.save(invitation);
    this.logger.log(
      `Invitation ${invitation.id} marked as DECLINED by user ${decliningUser.email}`,
    );
  }
  ...
  async cancelSentInvitation(
    ...
    await this.invitationsRepository.save(invitation);
    this.logger.log(
      `Invitation ${invitationId} marked as CANCELLED by user ${cancellingUser.id}`,
    );
    return invitation;
  }
  ...
  async getInvitationDetailsByToken(
    token: string,
  ): Promise<Partial<Invitation> & { householdName?: string }> {
    const invitation = await this.findByToken(token);
  ...
  async getPendingInvitationsForHousehold(
    householdId: string,
    requestingUser: User,
  ): Promise<Invitation[]> {
    this.logger.log(
      `User ${requestingUser.id} fetching PENDING invitations for household ${householdId}`,
    );

    // Authorization: Ensure the requesting user is part of the household or an owner
    if (
      requestingUser.householdId !== householdId ||
