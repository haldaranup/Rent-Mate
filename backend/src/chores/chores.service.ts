...
export class ChoresService {
  ...
  async create(createChoreDto: CreateChoreDto, creatingUser: User): Promise<Chore> {
    if (!creatingUser.householdId) {
      throw new BadRequestException('User must belong to a household to create chores.');
    }

    if (createChoreDto.assignedToId) {
      const assignedUser = await this.usersService.findOneById(createChoreDto.assignedToId);
      if (!assignedUser || assignedUser.householdId !== creatingUser.householdId) {
        throw new BadRequestException('Assigned user not found or does not belong to the same household.');
      }
    }
    ...
    const choreToCreate = {
      ...
      dueDate: createChoreDto.dueDate ? new Date(createChoreDto.dueDate) : undefined,
    };

    const savedChore = await this.choresRepository.save(this.choresRepository.create(choreToCreate));
  ...
  async findAllForHousehold(householdId: string, user: User): Promise<Chore[]> {
    if (user.householdId !== householdId) {
        throw new UnauthorizedException('You can only view chores for your own household.')
    }
  ...
  async findOne(id: string, user: User): Promise<Chore> {
    ...
    if (chore.householdId !== user.householdId) {
      throw new UnauthorizedException('You do not have permission to view this chore.');
    }
    return chore;
  }

  async update(id: string, updateChoreDto: UpdateChoreDto, user: User): Promise<Chore> {
    ...
    if (updateChoreDto.assignedToId !== undefined) {
        if (updateChoreDto.assignedToId === null && chore.assignedToId !== null) {
        ...
        } else if (updateChoreDto.assignedToId && updateChoreDto.assignedToId !== chore.assignedToId) {
            const assignedUser = await this.usersService.findOneById(updateChoreDto.assignedToId);
            ...
            if(originalChoreState.assignedToId) logDetails.previouslyAssignedTo = originalChoreState.assignedToId;
        }
    }
    ...
    if (updateChoreDto.isComplete !== undefined && updateChoreDto.isComplete !== chore.isComplete) {
        if (updateChoreDto.isComplete) {
            if (chore.recurrence && chore.recurrence !== ChoreRecurrence.NONE) {
                ...
                await this.activityLogService.createLogEntry({
                    ...
                    details: { via: 'update', description: chore.description, completedBy: user.id },
                });
                return rotatedChore;
            } else {
            ...
            if (user.role !== UserRole.OWNER && chore.completedById !== user.id && chore.completedById !== null) {
                throw new ForbiddenException('Only the household owner or the user who completed the chore can mark it as incomplete.');
            }
    ...
    if (updateChoreDto.description && updateChoreDto.description !== chore.description) {
        logDetails.descriptionChanged = { old: chore.description, new: updateChoreDto.description };
        chore.description = updateChoreDto.description;
        if (!activityTypeToLog) activityTypeToLog = ActivityType.CHORE_UPDATED;
    }
    if (updateChoreDto.notes !== undefined && updateChoreDto.notes !== chore.notes) {
        logDetails.notesChanged = { old: chore.notes, new: updateChoreDto.notes };
        chore.notes = updateChoreDto.notes;
        if (!activityTypeToLog) activityTypeToLog = ActivityType.CHORE_UPDATED;
    }
    if (updateChoreDto.recurrence && updateChoreDto.recurrence !== chore.recurrence) {
        logDetails.recurrenceChanged = { old: chore.recurrence, new: updateChoreDto.recurrence };
  ...
  private async completeAndRotateChore(chore: Chore, completingUser: User): Promise<Chore> {
    const originalChoreId = chore.id; // For logging original completion
    const originalChoreDescription = chore.description;

    if (!chore.householdId) throw new BadRequestException('Chore is not associated with a household.');
    const household = await this.householdsService.findOneByIdWithMembers(chore.householdId);
    if (!household || !household.members || household.members.length === 0) {
        ...
        await this.activityLogService.createLogEntry({
            ...
            details: { description: nonRotatedChore.description, completedBy: completingUser.id, note: 'Not rotated, no members.' }
        });
    ...
    if (members.length > 0) {
        const currentAssigneeIndex = chore.assignedToId ? members.findIndex(m => m.id === chore.assignedToId) : -1;
    ...
    switch (chore.recurrence) {
        case ChoreRecurrence.DAILY: nextDueDate = addDays(lastEffectiveDate, 1); break;
        case ChoreRecurrence.WEEKLY: nextDueDate = addWeeks(lastEffectiveDate, 1); break;
        case ChoreRecurrence.BI_WEEKLY: nextDueDate = addWeeks(lastEffectiveDate, 2); break;
        case ChoreRecurrence.MONTHLY: nextDueDate = addMonths(lastEffectiveDate, 1); break;
        default: nextDueDate = chore.dueDate; break;
    }
    ...
    await this.activityLogService.createLogEntry({
        ...
        entityType: 'Chore',
        activityType: ActivityType.CHORE_COMPLETED,
        details: { description: originalChoreDescription, completedBy: completingUser.id, recurrence: chore.recurrence },
    });
  ...
  async toggleComplete(id: string, user: User): Promise<Chore> {
    ...
    let logDetails: Record<string, any> = { description: chore.description };
  ...
  async getChoreCounts(householdId: string, user: User): Promise<{ total: number; completed: number; pending: number }> {
    if (user.householdId !== householdId) {
      throw new UnauthorizedException('Cannot get chore counts for this household.');
    }
    const total = await this.choresRepository.count({ where: { householdId } });
    const completed = await this.choresRepository.count({ where: { householdId, isComplete: true } });
    return { total, completed, pending: total - completed };
  }

  async getAssignedChoresForUser(userId: string, requestingUser: User): Promise<Chore[]> {
    if (userId !== requestingUser.id && requestingUser.role !== UserRole.OWNER) {
        throw new ForbiddenException('You can only view your own assigned chores or all if you are an owner.');
    }
    const targetUser = await this.usersService.findOneById(userId);
    if (!targetUser || !targetUser.householdId || targetUser.householdId !== requestingUser.householdId) {
  ...
  async getUnassignedChores(householdId: string, user: User): Promise<Chore[]> {
    if (user.householdId !== householdId) {
        throw new UnauthorizedException('Cannot view unassigned chores for this household.');
    }
    return this.choresRepository.find({
